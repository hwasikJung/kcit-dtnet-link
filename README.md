...
> UI 표기 정책: 비개발자 대상 화면이므로 사용자에게 보이는 문구에는 "API" 대신 **"기능"**으로 표기합니다(개발 문서·코드 식별자는 API 유지). 예외적으로 각 화면의 호출 기능 표기 라벨은 "이용 API"를 사용합니다.

## 주요 기능

| 메뉴 | 경로 | 설명 |
|---|---|---|
| 표준연계키 생성 | `/` | 건물 주소 입력(자동완성 지원) → 주소 정제·건축물대장 매칭을 거쳐 표준연계키(건축물대장 PK) 생성. 3단계 파이프라인·이용 API 표기, 키 카드(복사·요약 복사·링크 복사, 매칭 등급 한글 설명 툴팁, 위치 좌표·미니 지도·카카오맵 링크), 건축물대장 정보·신규 PK 전환 모달(페이지 이동 없이 확인·복사), 표제부 동 정보 자동 조회(총괄 다건이면 총괄별 그룹 표시). 입력창에 대장 PK(구형·신형)나 표준연계키(`R_`/`T_`/`S_`)를 직접 넣으면 생성 대신 조회·전환 액션과 소속 건물 목록 제시(신규 PK는 기존 PK 자동 전환 후 대장 정보 모달). 키 카드에 표준연계키(`R_`/`T_`/`S_`) 자동 표기. 초기화 버튼으로 입력·결과 일괄 소거. `?addr=<주소>` 링크로 결과 공유 가능. 최근 생성 이력(localStorage 최근 10건) 클릭 시 자동 재생성 |
| 표준연계키 일괄처리 | `/bulk` | 탭 2개 — ① **주소기반 일괄처리**: 엑셀 A열의 주소 업로드 → 표준연계키 일괄 생성 + 대표 PK로 표준연계키 조회 컬럼 병합 ② **PK기반 일괄처리**: A열의 표준연계키(`R_`/`T_`/`S_` 접두) 또는 대장 PK(구형·신형) 업로드 → `std_link_key` 표준연계키 정보 일괄 조회(입력 형식에 맞는 조회 파라미터 자동 선택). 공통: 파일 없이 텍스트 붙여넣기 입력(엑셀 표 복사 호환), 동시 5건·최대 5,000행·처리 중단(진행분 보존), 결과 테이블(상태 요약 칩 필터)·행 클릭 상세 Modal·xlsx 다운로드(업로드 파일의 B열~ 원본 컬럼 유지, 결과 컬럼은 그 뒤에 추가, "요약" 시트 동봉, 다운로드 전 컬럼 선택)·실패 행만 재시도, 브라우저 IndexedDB 이력(최근 20건, 처리 종류 배지 구분, 이름 붙이기, JSON 파일 내보내기/가져오기) |
...
공통: 헤더에 연계 서버 연결 상태 점(60초 주기 확인, 클릭 시 즉시 재확인, 장애 시 "서버 연결 오류" 표시).

## 기술 스택

- **Nuxt 4** (Vue 3.5) + TypeScript
- **shadcn-vue**(reka-ui) + **Tailwind CSS 4** + Pinia
- 엑셀 처리: SheetJS(xlsx) / JSON 뷰어: vue-json-pretty / 미니 지도: Leaflet + OpenStreetMap(펼칠 때만 동적 로드, 타일은 외부 인터넷 필요)
...
대상 서버에 표준 스펙 엔드포인트(`/v3/api-docs` 등)가 없어, `swagger-ui-init.js`의 인라인 스펙을 파싱해 `app/data/api-spec.json`에 정적 스냅샷으로 보관합니다(27개 엔드포인트, 10개 태그).
...
- [docs/저작권등록-프로그램설명서.md](docs/저작권등록-프로그램설명서.md) — 프로그램 저작권 등록용 설명서
