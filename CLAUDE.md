...
- **메뉴2 `/bulk` 표준연계키 일괄처리** (`app/pages/bulk.vue`) — 탭 2개. ① 주소기반 일괄처리: 엑셀 A열=주소 → 키 일괄 생성(매칭 응답의 총괄표제부 PK가 비면 첫 표제부 PK로 `std_link_key`를 후속 조회해 `mgm_upper_bld_pk`로 보강 — 실패 시 보강만 생략) ② PK기반 일괄처리: A열=표준연계키(`R_`/`T_` 접두) 또는 대장 PK(구형·신형) → `std_link_key` 일괄 조회(`detectStdLinkParam`이 입력 형식에 맞는 쿼리 파라미터 선택). 행 상세 Modal에서 총괄표제부 행(응답에 R 레코드)이면 "소속 표제부" 접이식 섹션 노출 — 펼칠 때만 `std_link_key`(표준연계키 파라미터)로 그룹 1회 조회해 보고 있는 총괄 소속 표제부만 필터 표시(원본 응답에 이미 그룹이 있으면 호출 생략, 이력에는 저장 안 함). 표제부 목록의 행을 클릭하면 기존 PK로 `mgm_bld_pk_info`를 지연 조회해 대장 정보(bulk-columns 재사용)를 인라인 확장 표시(PK별 캐시). 두 탭이 워커 풀(동시 5건, 최대 5,000행, **중단 버튼** — 진행분 저장·미처리 행 대기 유지)·결과 테이블(**상태 요약 칩 필터** — 이력 Modal 공용)/행 상세 Modal/xlsx 다운로드/실패 재시도·IndexedDB 이력(레코드 `kind`로 종류 구분, 최근 20건, **JSON 내보내기/가져오기**)을 공유. 파일 없이 **텍스트 붙여넣기** 입력 가능(`pasteToAoa` — 엑셀 표 복사의 탭 구분 열 보존). 업로드 파일의 B열~ 원본 컬럼은 결과 엑셀에 그대로 보존된다(`extraHeaders`/`BulkRow.extra` — 후처리 없이 원본 파일에 결과 컬럼이 붙은 형태)
...
테스트 대상은 UI가 아닌 순수 로직이다: 키 생성 응답 파싱·PK 형식 감지·좌표 추출(`app/lib/keygen.ts`), 주소 시트 파싱·키 컬럼 매핑(`app/lib/keygen-bulk.ts`), 엑셀/붙여넣기 파싱·헤더 감지(`app/lib/bulk-parse.ts`), 컬럼 매핑(`app/lib/bulk-columns.ts`), 표준연계키 조회 파싱(`app/lib/std-link-key.ts`), 매칭 등급 설명(`app/lib/match-grade.ts`), 이력 파일 직렬화·검증(`app/lib/history-io.ts`), 스펙 파서 헬퍼(`scripts/extract-spec-lib.mjs` — 실행 본문과 분리된 모듈). 새 로직도 이 패턴대로 lib/모듈로 분리해 테스트한다. `app/components/ui/`(shadcn 생성 코드)와 `app/data/`(생성 데이터), `*.md`는 lint/format 대상에서 제외되어 있다.
...
- **메뉴2 표준연계키 조회** `app/lib/std-link-key.ts` — `std_link_key` 응답(레코드 배열, 다건이면 컬럼별 중복 제거 후 콤마 연결) → 표시 컬럼(`STD_LINK_COLUMNS`), 입력 형식별 쿼리 파라미터 감지(`detectStdLinkParam` — `R_`/`T_` 접두=`std_link_key`, 하이픈 포함=구형 `mgm_bld_pk`, 하이픈 없음=신형 `mgm_bld_pk_new`). 미매칭 응답은 HTTP 200 + `{"error":"Cannot match"}`
